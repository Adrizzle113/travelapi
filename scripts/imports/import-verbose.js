...
async function importHotelInfo() {
  ...
  console.log('Opening file: ./dumps/hotel_info_en.json');
  ...
  for await (const line of rl) {
    ...
    if (lineNum % 1000 === 0) {
      console.log('Read ' + lineNum + ' lines, collected ' + hotels.length + ' hotels');
    }
    ...
    try {
      ...
      if (hotels.length >= BATCH_SIZE) {
        console.log('Inserting batch of ' + hotels.length + ' hotels...');
        ...
        count += hotels.length;
        console.log('SUCCESS! Processed ' + count + ' total hotels');
        hotels = [];
  ...
  console.log('File reading complete. Processing final batch...');
...
async function main() {
  console.log('HOTEL DATA IMPORT - VERBOSE');
